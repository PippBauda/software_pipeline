# Software Development Pipeline Formal Model — v3.0
...
- **Orchestrator-managed loop**:
  ...
  4. For each module (in dependency order):
     ...
     b. Dispatch commit: `[O3] Dispatching Builder for module <module-name> (M/N)`
...
- **Validation criteria**:
  ...
  - if `manifest.json` exists, its `schema_version` is verified against expected value `"3.0"`
...
1. **Context reconstruction**: the orchestrator reads `manifest.json` (pipeline state), artifacts from the current and preceding stages, and last relevant conversation logs
2. **Dispatch commit**: the orchestrator updates `manifest.json` setting `current_state` to `<STAGE>_IN_PROGRESS`, then commits with message `[<stage-id>] Dispatching to <agent-name>`
3. **Invocation**: the orchestrator invokes the specialized agent (as declared in each stage's **Agent** field), transmitting as input: formal stage artifacts, a context brief summarizing relevant conversation, any user feedback notes
4. **Agent work**: the agent produces the artifacts and returns the result to the orchestrator. The agent does NOT commit or update the manifest — these are orchestrator responsibilities (steps 5–6).
...
5. **Delegation**: the orchestrator identifies the agent responsible for the target stage from the Agent-to-Stage mapping and delegates to that agent following R.1 (starting from step 2, dispatch commit). The orchestrator MUST NOT execute stages assigned to other agents.
...
- **Commit messages**: format `[<stage-id>] <description>`. All commits are executed by the orchestrator. For stage completion commits where artifacts were produced by a subagent, the agent name is included as `[<stage-id>] [<agent-name>] <description>` to identify the author of the work. Examples:
  ...
  - `[RE-ENTRY] Return to O3 — artifacts archived in archive/20260316T120000/`
...
**Field descriptions**:
- `schema_version`: manifest schema version (`3.0`), verified by B1 for compatibility
...
O8_CICD_CONFIGURED       → O9_IN_PROGRESS                  [orchestrator starts O9]
...
*End of formal pipeline model v3.0.*
