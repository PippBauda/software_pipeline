...
4. **Agent work**: agent writes artifacts to disk, returns structured summary only. For C2: require `status` + `blocking_gaps` + `open_questions` + `assumptions` + `intent_version`.
...
6. **Executive summary**: brief summary for user based on agent's returned summary. Reference full report on disk. Do NOT read full reports into context.
