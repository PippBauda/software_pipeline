# Software Development Pipeline Formal Model — v2.0
...
- **Input**:
  ...
  - `docs/constraints.md`
- **Output**:
  - `docs/environment.md` — environment specification: required runtimes (language, version), dependencies (with lockfile), environment variables, build tools, recommended external tools (linters, SAST scanners, dependency auditors) for use in later stages (O5, O8)
...
- **Output** (on completion):
  - `logs/builder-cumulative-report-1.md` — cumulative report
...
- **User gate**: the user chooses between:
  - **Iteration**: re-entry at a specific pipeline point (C2–O9) providing instructions. On re-entry the **Re-Entry Protocol** (R.5) is applied. The re-entry point is validated by the orchestrator (see S.1 under State Machine).
...
2. The orchestrator assigns the task to the specialized agent (as declared in each stage's **Agent** field), transmitting as input: formal stage artifacts, a context brief summarizing relevant conversation, any user feedback notes
...
5. The orchestrator updates `manifest.json` with: completed stage, timestamp, produced artifacts, commit hash, responsible agent, progress metrics (see R.8)
...
1. **Return to O3**: the correction notes from the originating stage are passed to the Builder
2. **Re-execution from O4**: after O3 completes the corrections, the flow resumes from O4 (System Validation) and proceeds sequentially through all subsequent validation stages until reaching the stage that originated the correction
...
- **Sub-stage progress** (O3 only): during module generation, the manifest additionally records `progress.modules_completed` and `progress.modules_total`
- **Executive summary**: each executive summary (R.1 point 6) includes current progress (e.g., "Stage 12/19 — Module 3/8 completed")

---

# Manifest Schema

The `pipeline-state/manifest.json` file follows this schema:
...
**Field descriptions**:
- `schema_version`: manifest schema version, verified by B1 for compatibility
...
- `current_state`: the current state from the state machine
- `progress`: real-time progress tracking (see R.9)
- `stages_completed`: ordered array of all completed stage records. `execution_index` is incremented when a stage is re-executed (revision cycle or correction loop)
- `re_entries`: history of all re-entry events (R.5)
- `corrections`: history of all correction loops (R.7)
...
C9_IMPLEMENTATION_PLANNED → O1_ENVIRONMENT_READY           [after handoff check]
O1_ENVIRONMENT_READY     → O2_SCAFFOLD_CREATED
O2_SCAFFOLD_CREATED      → O3_MODULES_GENERATED
O3_MODULES_GENERATED     → O4_SYSTEM_VALIDATED
O4_SYSTEM_VALIDATED      → O3_MODULES_GENERATED            [correction — R.7]
O4_SYSTEM_VALIDATED      → O5_SECURITY_AUDITED
O5_SECURITY_AUDITED      → O3_MODULES_GENERATED            [correction — R.7]
O5_SECURITY_AUDITED      → O6_DEBUG_COMPLETED
O6_DEBUG_COMPLETED       → O3_MODULES_GENERATED            [correction — R.7]
...
COMPLETED                → any state C2–O9                 [re-entry — R.5, validated]
...
*End of formal pipeline model v2.0.*
